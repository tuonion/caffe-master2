...
void Solver<Dtype>::Step(int iters) {
	...
	if (gb_logDataFlag || gb_logFlag)
		stop_iter = 1000;
	LOG(INFO)<<"iters:"<<stop_iter;
	while (iter_ < stop_iter) {
		// zero-init the params
		net_->ClearParamDiffs();
		if (param_.test_interval() && iter_ % param_.test_interval() == 0 && (iter_ > 0 || param_.test_initialization()) && Caffe::root_solver()) {

			if (gb_logDataFlag == false && gb_logFlag == false) {
				struct timeval t1, t2;
				gettimeofday(&t1, NULL);
				//TestAll();
				gettimeofday(&t2, NULL);
				double used = (t2.tv_sec - t1.tv_sec) * 1000 + (t2.tv_usec - t1.tv_usec) / 1000.0;
				LOG(INFO)<<"Test #"<<testedNum <<" End. Used Time:"<<used <<" (ms)";
				testedNum++;
			}
		...
		if (display) {
			...
			for (int j = 0; j < result.size(); ++j) {
				...
				for (int k = 0; k < result[j]->count(); ++k) {
					...
					LOG_IF(INFO, Caffe::root_solver()) << "    Train net output #" << score_index++ << ": " << output_name << " = " << result_vec[k] << loss_msg_stream.str();
				}
			}
		}
		for (int i = 0; i < callbacks_.size(); ++i) {
		...
		if ((iter_ > 0) && (iter_ % 500 == 0)) {
			gettimeofday(&toc, NULL);
			timeUsed = (toc.tv_sec - tic.tv_sec) * 1000 + (toc.tv_usec - tic.tv_usec) / 1000.0;
			LOG(INFO)<<iter_<<" End. Time:"<<timeUsed << "(ms)";
			trainedEpoch++;
		}
...
void Solver<Dtype>::Restore(const char* state_file) {
	...
	if (state_filename.size() >= 3 && state_filename.compare(state_filename.size() - 3, 3, ".h5") == 0) {
